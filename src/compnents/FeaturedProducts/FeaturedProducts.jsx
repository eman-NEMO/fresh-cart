...
export default function FeaturedProducts() {
  ...
  let { addToCart,addToWishList,color,setColor,removedColor,setRemovedColor } = useContext(CartContext);
        ...
        <div className=" container">
          ...
          <div className="row">
            {data?.data.data.map((product) => (
              ...
              {color.includes(product._id)&&(!removedColor.includes(product._id))?
               <div className="float-end"> <p  className={`${Style.heartt}`} onClick={()=>addwish(product._id)}>  <i className=" fa fa-heart ms-5"></i></p></div>: 
               
               <div className="float-end"> <p   onClick={()=>addwish(product._id)}>  <i className=" fa fa-heart ms-5"></i></p></div>
